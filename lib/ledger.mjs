...
// godcode + super-godcode SKILL.md contents AND the lib modules (non-test .mjs, benchmark
// excluded) — every row is attributable to a harness revision covering all run behaviour.
...
// everything that changes run behaviour: both SKILL.md files + the lib modules themselves
// (not tests, not the benchmark rig — those don't alter live runs)
export function defaultHarnessFiles() {
  const libFiles = fs.readdirSync(HERE)
    .filter((f) => f.endsWith('.mjs') && !f.endsWith('.test.mjs'))
    .map((f) => path.join(HERE, f));
  return [...DEFAULT_SKILL_FILES, ...libFiles].sort();
}

export function harnessRev(skillFiles = null) {
  skillFiles = skillFiles ?? defaultHarnessFiles();
...
export function appendRow(row, { ledgerPath = DEFAULT_LEDGER, skillFiles = null } = {}) {
  validateRow(row);
