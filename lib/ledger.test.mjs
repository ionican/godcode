...
import { harnessRev, defaultHarnessFiles, appendRow, queryLedger, annotateRow, validateRow } from './ledger.mjs';
...
test('defaultHarnessFiles: covers both SKILL.md files AND the lib modules, not tests or benchmark', () => {
  // 2026-07-03 AR-fold gap: lib behaviour changed under an unchanged rev — the rev must
  // cover everything that changes run behaviour, or retro comparisons are polluted
  const files = defaultHarnessFiles();
  const names = files.map((f) => path.basename(f));
  assert.ok(files.some((f) => f.endsWith('skills/godcode/SKILL.md')), 'godcode SKILL.md');
  assert.ok(files.some((f) => f.endsWith('skills/super-godcode/SKILL.md')), 'super-godcode SKILL.md');
  assert.ok(names.includes('ledger.mjs'), 'ledger.mjs');
  assert.ok(names.includes('audit-run.mjs'), 'audit-run.mjs');
  assert.ok(names.includes('gate-runner.mjs'), 'gate-runner.mjs');
  assert.ok(!names.some((n) => n.endsWith('.test.mjs')), 'tests excluded');
  assert.ok(!files.some((f) => f.includes('/benchmark/')), 'benchmark rig excluded (does not affect live runs)');
  assert.deepEqual(files, [...files].sort(), 'stable order');
  assert.match(harnessRev(), /^[0-9a-f]{12}$/); // default rev computes over the full set
});
